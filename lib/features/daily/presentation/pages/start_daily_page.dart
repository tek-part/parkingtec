...
import 'package:parkingtec/features/auth/presentation/widgets/simple_logout_dialog.dart';
...
class StartDailyPage extends ConsumerStatefulWidget {
  const StartDailyPage({super.key});

  @override
  ConsumerState<StartDailyPage> createState() => _StartDailyPageState();
}

class _StartDailyPageState extends ConsumerState<StartDailyPage>
  ...
  Future<void> _startDaily() async {
    ...
    final startBalanceText = _startBalanceController.text.trim();
    if (startBalanceText.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            S.of(context).startBalanceRequired,
    ...
    final startBalance = double.tryParse(startBalanceText);
    if (startBalance == null || startBalance < 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            S.of(context).invalidStartBalance,
            style: const TextStyle(color: Colors.white),
          ),
          backgroundColor: AppColors.error,
        ),
    ...
    // Call controller to start daily
    await ref
        .read(dailyControllerProvider.notifier)
        .startDaily(startBalance, notes.isEmpty ? null : notes);
  }
  ...
  void _showLogoutDialog() {
    showDialog(
      context: context,
      builder: (context) => const SimpleLogoutDialog(),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<DailyState>(dailyControllerProvider, (previous, next) {
      next.maybeWhen(
        loaded: (daily, _) {
          // Success: reverse animation then navigate
          if (daily != null) {
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: Center(
                        child: Text(
                          S.of(context).startDailySession,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    AmountField(
                      ...
                      label: S.of(context).startBalance,
                    ...
                    ElevatedButton(
                      onPressed: isLoading ? null : _startDaily,
